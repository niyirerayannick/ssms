from django.urls import path
from . import views

app_name = 'finance'

urlpatterns = [
    path('dashboard/', views.finance_dashboard, name='dashboard'),
    path('school-fees/', views.school_fees_dashboard, name='school_fees_dashboard'),
    path('school-fees/bulk-entry/', views.bulk_fee_entry, name='bulk_fee_entry'),
    path('school-fees/disbursements/', views.fee_disbursement_queue, name='fee_disbursement_queue'),
    path('school-fees/disbursements/sync/', views.sync_fee_disbursement_queue, name='sync_fee_disbursement_queue'),
    path('school-fees/disbursements/export/', views.export_fee_disbursement_excel, name='export_fee_disbursement_excel'),
    path('school-fees/disbursements/export-pdf/', views.export_fee_disbursement_pdf, name='export_fee_disbursement_pdf'),
    path('school-fees/disbursements/mark-paid/', views.mark_fee_disbursements_paid, name='mark_fee_disbursements_paid'),
    path('school/<int:school_id>/students/', views.school_fee_students, name='school_fee_students'),
    path('export/excel/', views.export_fees_excel, name='export_fees_excel'),
    path('', views.fees_list, name='fees_list'),
    path('add/', views.fee_create, name='fee_create'),
    path('<hashid:pk>/edit/', views.fee_edit, name='fee_edit'),
    path('overdue/', views.overdue_fees, name='overdue_fees'),
    path('student/<hashid:student_id>/payments/', views.student_payment_history, name='student_payments'),
    path('student/<hashid:student_id>/add/', views.add_student_payment, name='add_student_payment'),
    path('api/student/<hashid:student_id>/details/', views.get_student_details, name='api_student_details'),
    path('api/family/<hashid:family_id>/insurance/', views.get_family_insurance_details, name='api_family_insurance'),
    path('api/district/<hashid:district_id>/families/', views.get_families_by_district, name='api_families_by_district'),
    path('api/district/<hashid:district_id>/students/', views.get_students_by_district, name='api_students_by_district'),
]

